...
async function getUserData() {
    ...
    if (!user_data || Object.keys(user_data).length === 0) {
        // Mock fallback to allow immediate viewing of page layout and design
        user_data = {
            userId: 1,
            username: "@guest_user",
            first_name: "Guest",
    ...
    document.querySelectorAll('.myProfilePic').forEach(image => {
        if (user_data.dp.startsWith('http') || user_data.dp.startsWith('/assets/')) {
            image.src = user_data.dp;
        } else {
    ...
    if (createPostInput) {
        createPostInput.placeholder = `What's on your mind, ${user_data.first_name}?`;
    }
}
