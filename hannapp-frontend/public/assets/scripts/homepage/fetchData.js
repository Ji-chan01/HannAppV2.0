...
async function fetchPosts() {
    ...
    if (!user_data) {
        user_data = { userId: 1, username: "@guest_user", full_name: "Guest Visitor", dp: "/assets/images/dp/avatar-men.png" };
    }
    try {
        ...
        console.warn("Backend API offline. Loading sample mock posts fallback...", error.message);
        loadMockPosts();
    } finally {
        isLoading = false;
    }
...
export async function addEventListeners(node) {
    ...
    node.querySelectorAll(".profile-link").forEach(link => {
        link.addEventListener("click", async () => {
            ...
            window.location.href = `/profile?p=${profileId}`;
        })
    })
...
async function getFriends() {
    ...
    try {
        ...
        if (!data.friends || data.friends.length === 0) {
            throw new Error("No friends loaded");
        }
        ...
        console.warn("Backend API offline. Loading sample following profiles fallback...", e.message);
        loadMockFriends();
    }
}
    ...
    const samplePosts = [
        {
            ...
            comments: [
                {
                    user: { full_name: "John Carter", dp: "/assets/images/dp/avatar-men.png" },
                    content: "This looks absolutely breathtaking, Hannah! Enjoy the getaway!",
                    commentAt: new Date(Date.now() - 3600000).toISOString()
                }
            ]
        },
        {
    ...
    samplePosts.forEach((post) => {
        ...
        node.innerHTML = `
            <div class="feed-header">
                <div class="dp-container">
                    <div class="profile-picture">
                        <img src="${post.profile_picture}" alt="" loading="lazy">
                    </div>
                </div>
                <div class="poster-container-info">
                    <div class="feed-header-container">
                        <div class="poster-infos">
                            <div class="name">
                                <a href="#" data-profile-id="${post.post_id}" class="profile-link"><h4>${post.full_name}</h4></a>
                                <p class="text-muted">&bull;</p>
                                <p class="text-muted">${timeConversion(post.postAt)}</p>
                            </div>
                            <div class="time">
                        ...
                        <p href="#">
                            <span class="edit" data-edit-id="${post.post_id}">
                                <span class="options"></span>
                            </span>
                        </p>
                    </div>
                    <div class="photo">
                        ...
                        <div class="facebook-post">
                            ...
                            <div class="image-grid single-image">
                                <img class="photo-post" src="${post.image}" loading="lazy" alt="">
                            </div>
                        </div>
                        <div class="reactions">
                            <div>
        ...
        // Add like handler for mock post
        const likeBtn = node.querySelector('.like');
        const likeCount = node.querySelector('.like + h1');
        ...
        // Add comments click handler
        const commentBtn = node.querySelector('.comment');
        if (commentBtn) {
                            ...
                            post.comments.forEach(comment => {
                                ...
                                commentNode.innerHTML = `
                                    <div class="comment-details">
                                        ...
                                        <div class="comment-body">
                                            <div><a href=""><h5>${comment.user.full_name}</h5></a><p class="text-muted" style="font-size: 0.7rem;">Just now</p></div>            
                                            <p>${comment.content}</p>
                                            <p class="text-muted">Reply</p>
                                        </div>
                                    </div>
...
function loadMockFriends() {
    ...
    if (friendContainer.children.length > 0) return;
